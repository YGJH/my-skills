---
name: gpu-batched-env-simulation
description: Use when designing a GPU-accelerated RL environment where thousands of instances run in parallel on CUDA kernels, when moving env logic (physics, action decoding, observation encoding) to GPU, or when asked about batched env simulation architecture, GPU-resident state, forecast-based observations, or fusing step+obs+decode into CUDA kernels
---
